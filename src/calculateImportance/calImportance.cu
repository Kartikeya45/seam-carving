 ...
 */
void calImportance(uint8_t *in1, uint8_t *in2, int height, int width, uint8_t *out, bool useDevice, dim3 blockSize) {
	GpuTimer timer;
	timer.Start();

	if (useDevice == false) 
        addMatrix_host(in1, in2, height, width, out);
	else { 
        ...
        addMatrix_kernel<<<gridSize, blockSize>>>(d_in1, d_in2, height, width, d_out);
